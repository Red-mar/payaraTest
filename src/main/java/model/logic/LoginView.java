...
public class LoginView implements Serializable {
    ...
    public String login() {
        ...
        log.log(Level.INFO, "Database now");
        this.user = facade.getPersonByName(principal.getName());
        log.info("Authentication done for user: " + principal.getName());
        ExternalContext externalContext = FacesContext.getCurrentInstance().getExternalContext();
        Map<String, Object> sessionMap = externalContext.getSessionMap();
        sessionMap.put("User", user);
        if (user.getRole().equals(ADMIN)) {
            log.info("returning to adminonly");
            return "/admin/adminonly?faces-redirect=true";
        } else if (user.getRole().equals(USER)) {
            log.info("returning to useronly");
            return "/user/useronly?faces-redirect=true";
        } else {
    ...
    public PersonDO getAuthenticatedUser() {
        return user;
    }
